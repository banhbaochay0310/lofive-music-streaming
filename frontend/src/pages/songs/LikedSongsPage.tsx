import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import { usePlayerStore } from "@/stores/usePlayerStore";
import { Clock, Pause, Play } from "lucide-react";
import { formatDuration } from "@/utils/formatDuration";
import LikeButton from "@/components/LikeButton";

const LikedSongsPage = () => {
  const { currentSong, isPlaying, playAlbum, togglePlay, likedSongs } =
    usePlayerStore();

  const handlePlayLikedSongs = () => {
    if (!likedSongs.length) return;

    const isCurrentLikedSongsPlaying = likedSongs.some(
      (song) => song._id === currentSong?._id
    );
    if (isCurrentLikedSongsPlaying) togglePlay();
    else {
      playAlbum(likedSongs, 0);
    }
  };

  const handlePlaySong = (index: number) => {
    if (!likedSongs.length) return;
    playAlbum(likedSongs, index);
  };

  return (
    <div className="h-full">
      <ScrollArea className="h-full rounded-md">
        {/* Main content */}
        <div className="relative min-h-screen">
          {/* Bg gradient */}
          <div
            className="absolute inset-0 bg-gradient-to-b from-[#5038a0]/80 via-zinc-900/80 to-zinc-900 pointer-events-none"
            aria-hidden="true"
          />

          {/* Content */}
          <div className="relative z-10">
            <div className="flex p-6 gap-6 pb-8">
              <img
                src="LikedSong.jpg"
                alt=""
                className="w-[240px] h-[240px] rounded shadow-xl"
              />
              <div className="flex flex-col justify-end">
                <p className="text-sm font-medium">Playlist</p>
                <h1 className="text-7xl font-bold my-4">Liked Songs</h1>
                <div className="flex items-center gap-2 text-sm text-zinc-100">
                  <span>• {likedSongs.length} songs</span>
                </div>
              </div>
            </div>

            {/* Play button */}
            <div className="px-6 pb-4 flex items-center gap-6">
              <Button
                onClick={handlePlayLikedSongs}
                size="icon"
                className="w-14 h-14 rounded-full bg-green-500 hover:bg-green-400 hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-green-500 disabled:hover:scale-100"
              >
                {isPlaying &&
                likedSongs.some((song) => song._id === currentSong?._id) ? (
                  <Pause className="h-10 w-10 text-black" fill="black" />
                ) : (
                  <Play className="h-10 w-10 text-black" fill="black" />
                )}
              </Button>
            </div>

            {/* Table section */}
            <div className="bg-black/20 backdrop-blur-sm">
              {/* Table header */}
              <div className="grid grid-cols-[16px_4fr_2fr_1fr] gap-4 px-10 py-2 text-sm text-zinc-400 border-b border-white/5">
                <div>#</div>
                <div>Title</div>
                <div>Added Date</div>
                <div>
                  <Clock className="h-4 w-4" />
                </div>
              </div>
              {/* Songs list */}
              <div className="px-6">
                <div className="space-y-2 py-4">
                  {likedSongs.length === 0 ? (
                    <div className="text-center py-8 text-zinc-400">
                      This album feels a little empty...
                    </div>
                  ) : (
                    likedSongs.map((song, index) => {
                      const isCurrentSong = currentSong?._id === song._id;
                      return (
                        <div
                          key={song._id}
                          onDoubleClick={() => handlePlaySong(index)}
                          className="grid grid-cols-[16px_4fr_2fr_1fr] gap-4 px-4 py-2 text-sm text-zinc-400 hover:bg-white/5 rounded-md group cursor-pointer"
                        >
                          <div className="flex items-center justify-center">
                            {isCurrentSong && isPlaying ? (
                              <>
                                <div className="size-5 text-green-500 group-hover:hidden">
                                  ♫
                                </div>
                                <Pause
                                  className="h-4 w-4 hidden group-hover:block"
                                  fill="white"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    togglePlay();
                                  }}
                                />
                              </>
                            ) : (
                              <span className="group-hover:hidden">
                                {index + 1}
                              </span>
                            )}
                            {isCurrentSong && !isPlaying ? (
                              <>
                                <div
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handlePlaySong(index);
                                  }}
                                >
                                  <Play
                                    className="h-4 w-4 hidden group-hover:block"
                                    fill="white"
                                  />
                                </div>
                              </>
                            ) : (
                              <></>
                            )}
                            {!isCurrentSong && (
                              <div
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handlePlaySong(index);
                                }}
                              >
                                <Play
                                  className="h-4 w-4 hidden group-hover:block"
                                  fill="white"
                                />
                              </div>
                            )}
                          </div>
                          <div className="flex items-center gap-3">
                            <img
                              src={song.imageUrl}
                              alt={song.title}
                              className="size-10"
                            />

                            <div>
                              <div className="font-medium text-white">
                                {song.title}
                              </div>
                              <div>{song.artist}</div>
                            </div>
                          </div>
                          <div className="flex items-center">
                            {new Date(song.createdAt).toLocaleDateString(
                              "en-US",
                              {
                                day: "numeric",
                                month: "short",
                                year: "numeric",
                              }
                            )}
                          </div>
                          <div className="flex items-center justify-between">
                            <div>{formatDuration(song.duration)}</div>
                            <div className="opacity-0 group-hover:opacity-100">
                              <LikeButton song={song} />
                            </div>
                          </div>
                        </div>
                      );
                    })
                  )}
                </div>
              </div>
            </div>
          </div>
        </div>
      </ScrollArea>
    </div>
  );
};

export default LikedSongsPage;
